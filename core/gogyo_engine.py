...
class GogyoEngine:
    ...
    def analyze_meishiki_gogyo(cls, meishiki_data):
        ...
        if (
            not has_day_stem_root
            and insei_score >= 4.0
            and insei_score > hibo_score * 2
        ):
            is_inta_mijaku = True
        ...
        if is_inta_mijaku:
            judgment = "身弱＊"
            judgment_note = "五行の集中（偏り）により構造上身弱となります。"
        elif is_tokurei and has_day_stem_root:
            judgment = "身強"
        elif (
            not is_tokurei
            and not has_day_stem_root
            and jitou_score < itau_score * 1.2
        ):
            judgment = "身弱"
        else:
        ...
        return {
            ...
            "judgment": judgment,
            "judgment_note": judgment_note,  # ★ UI連携用キーの追加
